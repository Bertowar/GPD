...
export const EntryForm: React.FC = () => {
  ...
  // Retrieve entry from location state safely
  const editEntryState = (location.state as any)?.editEntry as ProductionEntry | undefined;
  ...
  // Form State
  // CRITICAL: Store ID in state to persist across renders if location.state varies
  const [currentId, setCurrentId] = useState<string | null>(editEntryState?.id || null);
  ...
  useEffect(() => {
    if (productCode && machineId) {
        ...
        if (editEntryState && editEntryState.productCode === productCode && editEntryState.machineId === machineId) {
            return;
        }
        if (!isValid && !selectedOpId) setProductCode(null);
    }
  }, [machineId, filteredProducts, productCode, selectedOpId, editEntryState]);

  useEffect(() => {
    if (editEntryState) {
        setCurrentId(editEntryState.id);
        setDate(editEntryState.date);
        setMachineId(editEntryState.machineId);
        setOperatorId(editEntryState.operatorId);
        setShift(editEntryState.shift || '');
        setProductCode(editEntryState.productCode || null);
        setSelectedOpId(editEntryState.productionOrderId || '');
        setStartTime(editEntryState.startTime || '');
        setEndTime(editEntryState.endTime || '');
        
        // Load Fields
        setQtyOK(editEntryState.qtyOK > 0 ? editEntryState.qtyOK.toString() : '');
        
        setObservations(editEntryState.observations);
        setIsDraft(editEntryState.metaData?.is_draft === true);
        if (editEntryState.downtimeMinutes > 0) {
            setIsDowntime(true);
            setDowntimeTypeId(editEntryState.downtimeTypeId || '');
            setIsLongStop(editEntryState.metaData?.long_stop === true);
        }
        
        const weight = editEntryState.metaData?.measuredWeight || editEntryState.metaData?.peso_produto || '';
        setMeasuredWeight(String(weight));
        setCycleTime(editEntryState.metaData?.cycle_time ? String(editEntryState.metaData.cycle_time) : '');
        
        // Fix for number value in bobbin_weight preventing replace
        const bWeight = editEntryState.metaData?.bobbin_weight;
        setBobbinWeight(bWeight !== undefined && bWeight !== null ? String(bWeight) : '');

        setCustomValues(editEntryState.metaData || {});
    }
  }, [editEntryState]); 

  useEffect(() => {
      if (!machineId && !editEntryState) {
          setCurrentId(null);
          setOperatorId('');
          setProductCode(null);
          setSelectedOpId('');
  ...
  const processFormSubmit = async (shouldClose: boolean) => {
    ...
    // REGRAS DE NEGÓCIO:
    // 1. Ignora validação se for Parada Longa (sem fim definido).
    // 2. Ignora validação se for Edição (currentId existe), para permitir ajuste de apontamentos.
    if (!isLongStop && !currentId) {
    ...
    const metaPayload: any = { 
        ...customValues,
        measuredWeight: measuredWeight ? Number(String(measuredWeight).replace(',', '.')) : null,
        bobbin_weight: bobbinWeight ? Number(String(bobbinWeight).replace(',', '.')) : null,
        cycle_time: cycleTime,
        was_draft: editEntryState?.metaData?.is_draft === true,
        long_stop: isDowntime ? isLongStop : false,
        is_draft: isDraft 
    };

    // Safe UUID generation (Robust Polyfill for non-secure contexts)
    const genId = () => {
    ...
    // Use currentId if available (Edit Mode), else generate new
    const finalId = currentId || genId();
    ...
    const entry: ProductionEntry = {
        ...
        createdAt: editEntryState?.createdAt || Date.now()
    };

    await saveEntryMutation({ entry, isEdit: !!currentId });
    
    if (shouldClose) {
        if (currentId) navigate('/list');
        else closeModal();
    } else {
        ...
        if (isLongStop) {
            ...
            setObservations('');
            setCurrentId(null); // Reset ID for new entry
                    ...
                    <div className="flex flex-col md:flex-row items-center justify-between p-4 border-b border-slate-100 bg-slate-50 shrink-0 gap-4">
                        <div className="flex flex-col w-full md:w-auto">
                            <h3 className="text-sm font-bold text-slate-500 uppercase tracking-wide">
                                {currentId ? 'Editar Apontamento' : 'Novo Apontamento'}
                            </h3>
